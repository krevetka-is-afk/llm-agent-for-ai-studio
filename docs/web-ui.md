...
9. Следовать no-code CTA: открыть Agent Atelier, перенести показанные модель,
   инструкцию, tools и параметры генерации, затем создать постоянного агента.
10. Для интеграции с сайтом или ботом скачать ZIP-пакет и передать его
...
агента всегда доступны в sidebar. Автоматический импорт JSON в Agent Atelier не
обещается: основной путь MVP — понятный ручной перенос уже проверенных настроек.
