...
Минимальный diff, но сохраняет привязанные к репозиторию imports и размывает
границу между внешним и внутренним API. Отклонено.

### Big-bang rewrite в строгий Clean Architecture

Даёт визуально чистое дерево, но смешивает перенос, переименование и изменение
поведения в одной долгой ветке. Отклонено из-за риска и сложного review.
