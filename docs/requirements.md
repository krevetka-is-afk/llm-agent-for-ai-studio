...
| FR-19 | Preview Code Interpreter должен сохранять файлы текущего Builder-диалога между сообщениями, принимать дополнительные request-scoped файлы и возвращать локальные generated artifacts. | UI восстанавливает доверенные локальные handles из history, автоматически добавляет их только при наличии соответствующей capability и показывает uploader для дополнительных inputs; файлы загружаются с `purpose=user_data`, IDs привязываются к копии runtime, outputs скачиваются потоково, а известные input/output files и containers очищаются на success/error/timeout. |
