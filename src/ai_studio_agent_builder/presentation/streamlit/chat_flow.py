...
def _request_logger(user_id: str, request_id: str) -> logging.LoggerAdapter:
...
def _save_attachments(
    ...
    try:
        ...
        _request_logger(connection_id, request_id).exception(
...
def _append_and_render_assistant_message(
    ...
    with st.chat_message("assistant"):
        with st.spinner("Генерируется ответ..."):
            ...
            result_parts = list(result.parts) if result is not None else []
            if result_parts:
                render_result_parts(
                    result_parts,
...
def _request_answer(
    ...
    try:
        ...
        request_logger = _request_logger(connection_id, request_id)
        if isinstance(
            exc,
            (
                AgentSpecificationImportError,
