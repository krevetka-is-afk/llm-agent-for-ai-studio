"""Aiogram handlers for the experimental Telegram adapter."""
...
from .media import classify_message, download_media
from .messages import MessageService
from .request_gate import PerUserRequestGate
...
class StoredCredentials(Protocol):
    api_token: str | None
    folder_id: str | None


class TelegramUserStore(Protocol):
    def get(self, user_id: str) -> StoredCredentials: ...
...
def create_router(
    ...
    router = Router()
    handlers = TelegramHandlers(bot, ai_service, user_store)
    ...
    router.message.register(
        handlers.cmd_clear_credentials,
        Command(commands=["clear_credentials"]),
    )
    router.message.register(
        handlers.cmd_set_token,
        Command(commands=["set_api_token"]),
    )
    router.message.register(
        handlers.cmd_set_folder_id,
        Command(commands=["set_folder_id"]),
    )
...
class TelegramHandlers:
    ...
    async def universal_handler(self, message: types.Message) -> None:
        user_id = str(_require_from_user(message).id)
        async with self._request_gate.hold(user_id):
            conversation_state = self._user_store.get_state(user_id)
            ...
            base_dir = self._ai_service.user_files_dir(user_id)
            filename = None
...
async def _validate_pending_connection(
    ...
    try:
        ...
        logging.exception(
            "AI Studio connection validation failed for user_id=%s",
            user_id,
        )
        await message.reply(
    ...
    if deleted:
        ...
        await message.reply(
            "Подключение проверено. Не удалось удалить одно из сообщений — "
            "удалите их вручную."
        )


async def _delete_secret_messages(
    bot: TelegramMessageDeleter,
    chat_id: int,
...
async def _resolve_credentials(
    ...
    if manual.folder_id is None:
        await message.reply(
            MessageService.render_command_usage_msg(
                "/set_folder_id",
                "Перед запросом сначала сохраните folder id.",
            )
        )
        return None
    return AIStudioCredentials(
        api_key=manual.api_token,
