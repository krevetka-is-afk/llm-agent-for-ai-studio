...
- Provider-файлы векторного индекса не переиспользуются в Code Interpreter по
  `file_id`. Локальные исходники из Builder-чата UI может приложить к stateless
  preview заново; не обещай перенос provider state и не запрашивай `container_id`.
